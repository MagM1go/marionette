...
@pytest.mark.asyncio
async def test_enter_sets_location_for_character(
    ...
    result = await EnterLocationUseCase(repo, fake_transaction).enter(100, "Airi", 777)

    assert result.location_id == 777
    assert character.entered_channel_id == 777
    assert fake_transaction.commit_calls == 1
    assert fake_transaction.rollback_calls == 0


@pytest.mark.asyncio
async def test_enter_raises_when_character_missing(
    ...
    with pytest.raises(CharacterNotFound):
        await EnterLocationUseCase(repo, fake_transaction).enter(100, "Airi", 777)
...
@pytest.mark.asyncio
async def test_enter_raises_when_character_already_entered(
    character_factory: Callable[..., Character],
    character_repo_factory: Callable[..., FakeCharacterRepository],
    fake_transaction: FakeTransaction,
) -> None:
...
@pytest.mark.asyncio
async def test_enter_raises_when_another_character_is_active(
    character_factory: Callable[..., Character],
    character_repo_factory: Callable[..., FakeCharacterRepository],
    fake_transaction: FakeTransaction,
) -> None:
    target = character_factory(name="Airi")
    active = character_factory(name="Ren", entered_channel_id=321)
