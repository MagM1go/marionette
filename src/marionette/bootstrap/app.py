...
def _init_container(context: dict[Any, Any]) -> AsyncContainer:
    providers = (DatabaseProvider(), InfrastructureProvider(), UseCaseProvider(), RepositoryProvider(), QueryProvider())
...
def build_discord_bot(plugins_path: str) -> hikari.GatewayBot:
    ...
    dishka_container = _init_container(context={hikari.api.Cache: bot.cache, hikari.api.RESTClient: bot.rest})
