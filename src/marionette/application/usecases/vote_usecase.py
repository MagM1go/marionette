...
class VoteUseCase:
    ...
    async def vote_for(
        self, voted_by: UserId, user_id: UserId, character_name: str, now: datetime
    ) -> None:
        async with self._transaction:
            character = await self._character_repository.get_by_user_id_and_name(
            ...
            vote = await self._vote_repository.get_vote_by_character_id(
                character_id=CharacterId(character.id)
            )
            if vote is not None and VotePolicy.is_on_cooldown(vote=vote, now=now):
                raise VoteOnCooldown(
                    character.name, VotePolicy.cooldown_remaining(vote=vote, now=now)
                )
