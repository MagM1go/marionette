...
class MoveOnboardingToIntroUseCase(BaseOnboardingUseCase):
    async def execute(self, zone_id: int, user_id: UserId) -> None:
        async with self._transaction as transaction:
            ...
            await self._move(transaction, user_id, state, OnboardingStep.INTRO)

        await self._apply_step_assets(zone_id=zone_id, user_id=user_id, step=OnboardingStep.INTRO)
