...
from marionette.domain.exceptions import OnboardingRulesAlreadyAcceptedError


class AcceptOnboardingRulesUseCase(BaseOnboardingUseCase):
    async def execute(self, zone_id: int, user_id: UserId) -> None:
        async with self._transaction as transaction:
            ...
            if state.current_step >= OnboardingStep.REGISTRATION:
                raise OnboardingRulesAlreadyAcceptedError()

            await self._move(transaction, user_id, state, OnboardingStep.REGISTRATION)

        await self._apply_step_assets(
            zone_id=zone_id,
            user_id=user_id,
